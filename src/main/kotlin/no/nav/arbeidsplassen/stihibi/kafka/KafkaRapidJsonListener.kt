...
/**
 * Tilpasset CV. Her lyttes det på alle meldinger på et topic. Det filtreres ikke på eventer
 */
class KafkaRapidJsonListener(
    ...
    override fun handleRecord(record: ConsumerRecord<String?, ByteArray?>) = record.key()?.let { key ->
        try {
            ...
            val message = JsonMessage(key, eventId, record.value()?.let { String(it) }, record.timestamp(),
